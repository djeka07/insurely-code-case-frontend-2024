...
export const InsuranceListing = ({ insurances }: InsuranceListingProps) => {
  ...
  const other = insurances.filter((ins) => [...home, ...car, ...person, ...animal].indexOf(ins) === -1);
        ...
        <div className={styles.section} key={date}>
          ...
          <ul className={styles.list}>
            {groupedItems[date].map((ins: InsuranceResponse) => (
              <InsuranceCard key={ins.insurance.externalId} insurance={ins} className={styles.item} />
            ))}
          </ul>
        </div>
      ));
          ...
          <ParagraphPreamble className={styles.total}>
            ...
            {formatMessage('amount/month', {
              amount: formatCurrency(
                config,
                Math.ceil(list.items.reduce((acc, ins) => acc + (ins.insurance.premiumAmountYearRounded || 0), 0) / 12),
              ),
            })}
          </ParagraphPreamble>
